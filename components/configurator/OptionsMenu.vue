...
function optionClicked(index: number, label: string) {
  setCurrentOption(index, label);
}
</script>

<template>
  <div class="flex flex-wrap gap-3 overfl ow-y-auto sm:w-[660px]">
    <div
      v-for="(label, index) in optionLabels"
      :key="index"
      ...
      @click="optionClicked(index, label)"
    >
      <img
